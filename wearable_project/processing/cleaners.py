...
def parse_outer_timestamp(value: Any) -> tuple[str | None, datetime | None, set[str]]:
    # The outer datetime and backend lifecycle fields are defined by this export
    # as UTC bucket/backend timestamps even though they are serialized without
    # an explicit offset. Treat that as a known source convention rather than
    # flagging every valid event as an assumption. Parse failures still surface.
...
def standardize_time_fields(row: dict[str, Any], spec: FeatureSpec) -> None:
    ...
    if start_dt is not None and end_dt is not None:
        ...
        if spec.family in {FeatureFamily.POINT_SCALAR, FeatureFamily.POINT_VECTOR} and duration != 0:
            flags.add("point_feature_has_nonzero_duration")
        if spec.family in {FeatureFamily.INTERVAL_TOTAL, FeatureFamily.INTENSIVE_INTERVAL} and duration == 0:
            flags.add("interval_feature_has_zero_duration")
    else:
    ...
    if start_dt is not None and outer_dt is not None:
        ...
        if start_local:
            try:
                if datetime.fromisoformat(start_local).date() != outer_dt.date():
                    flags.add("outer_bucket_differs_from_local_start_date")
            except ValueError:
                pass
...
def apply_unit_policy(row: dict[str, Any], spec: FeatureSpec) -> None:
    ...
    if not explicit and policy.status.startswith("inferred"):
        flags.add("unit_inferred_not_explicit")
...
def acquisition_method(row: dict[str, Any], feature: str) -> str:
    if nullable_bool(row.get("was_user_entered")) is True:
        return "user_entered"
    sid = (text(row.get("source_id")) or "").lower()
    if feature == "BloodAlcoholContent" and "intellidrink" in sid:
        return "calculator_estimate"
    if sid:
        return "device_or_application_import"
    return "unknown"
...
def occurrence_detail(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "outer_id": text(row.get("outer_id")),
        "outer_row_number": integer(row.get("outer_row_number"), -1),
...
def revision_detail(row: dict[str, Any], spec: FeatureSpec) -> dict[str, Any]:
    return {
        "event_id": text(row.get("event_id")), "record_id": text(row.get("record_id")),
        "datetime": text(row.get("datetime")), "source_file": text(row.get("source_file")),
...
BASE_COLUMNS = [
    "event_id", "content_id", "participant_id", "feature", "data_source", "collecting_method_version",
    "record_id", "start_date", "end_date", "start_date_local", "end_date_local", "utc_offset_minutes",
    "duration_seconds", "datetime", "created_at", "updated_at", "source_id", "source_name", "device",
]
TAIL_COLUMNS = [
    "raw_unit", "canonical_value", "canonical_unit", "unit_status", "unit_evidence", "acquisition_method",
    "time_zone", "start_date_iana", "end_date_iana", "was_user_entered",
    "heart_rate_motion_context", "status", "trend_arrow", "trend_rate",
    "metadata_device_name", "metadata_sync_version", "vo2_max_test_type", "metadata", "metadata_raw",
    "occurrence_count", "duplicate_count", "revision_count", "duplicate_record_ids",
    "duplicate_details", "revision_details",
    "conflict_group_id", "quality_flags", "outer_id", "source_file", "source_month", "source_file_sha256",
    "outer_row_number", "payload_index", "start_date_raw", "end_date_raw", "datetime_raw", "created_at_raw",
    "updated_at_raw", "interval_group_id",
]


def output_dataframe(rows: list[dict[str, Any]], spec: FeatureSpec) -> pd.DataFrame:
    if spec.family == FeatureFamily.DAILY_SUMMARY:
        rows.sort(key=lambda row: (text(row.get("datetime")) or "", integer(row.get("payload_index"), -1), row["event_id"]))
    else:
        rows.sort(key=lambda row: (text(row.get("start_date")) or "", text(row.get("end_date")) or "", source_key(row) or "", text(row.get("record_id")) or "", row["event_id"]))
    frame = pd.DataFrame(rows)
    # Internal reconciliation keys are recomputed on every clean pass and do not
    # belong in the user-facing participant feature CSV. File hashes live in the
    # hidden state database rather than being repeated in every event row.
    frame = frame.drop(columns=[
        name for name in ("content_id", "interval_group_id", "source_file_sha256",
                          "created_at_raw", "updated_at_raw", "quality_flags_raw")
    ...
    extra_canonical = [name for name in ("canonical_systolic_value", "canonical_diastolic_value", "waveform_sample_count") if name in frame.columns]
    order: list[str] = []
    for name in [*BASE_COLUMNS, *measurement_columns, *extra_canonical, *TAIL_COLUMNS]:
        if name in frame.columns and name not in order:
            order.append(name)
    order.extend(name for name in frame.columns if name not in order and name != "quality_flags_raw")
